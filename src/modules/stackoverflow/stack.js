...
import { getStackAction } from './stack.action';
     ...
     return (
        <>
            <form onSubmit ={handleSubmit}>
                <input value={title} placeholder="wpisz temat" onChange={handleChange}/>
                <p>Sortuj według: </p>
                <input type="radio" id="creation" name="sort" value="creation" onChange={handleClick} checked={active === 'creation'} />  
                <label htmlFor="creation">daty</label>
                <input type="radio" id="votes" name="sort" value="votes" onChange={handleClick} checked={active === 'votes'}/>  
                <label htmlFor="votes">punktów</label>
                <input type="submit" value="Szukaj"/>
            </form>
            <section>
                {solutions.length>0 ? <ul>
                    {solutions.map(({question_id, creation_date, is_answered, score, link, title})=><li key={question_id}>{title}<ul>
