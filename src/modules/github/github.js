    ...
    return(
       <>
            <form onSubmit={handleSubmit}>
                <input onChange={handleChange} name="user" value ={user} placeholder="Nazwa użytkownika"/>
                <input type="submit" value="Szukaj" />
                <input name="filter" value={filter} onChange={handleChange} placeholder="filtruj"/>
               
            </form>
            {repos.length >0 ? <>
                <h3>Repozytorium:</h3>
                <ul>{repos.filter(item=>filterRepos(item)).map(item=><li key={item.id}>{item.name}</li>)}</ul> 
            </> :null }
